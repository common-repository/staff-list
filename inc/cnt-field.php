...
//== SINGLE PAGE TEXT LINK START SPTL =  Single Page Text Link. ====================================
function abcfsl_cnt_field_SPTL( $par, $itemOptns ){

    // Called from: abcfsl_cnt_txt_field. 
    // Out: sPg text link container + content.
    //-------------------------------------------------
    // Individual staff memers can hide sPg links. Staff member option.
    ...
    $parLP['lineTxt'] = $par['lineTxt']; //$tplateOptns['_sPgLnkTxt']
    $parLP['imgLnkLDefault'] = $par['imgLnkLDefault'];

    // sPg text link can use template's setting or staff member custom text.
    // When both of them are empty, no link will show up.
    //$itemTxt = $par['lineTxt'];
    $itemTxtCust = isset( $itemOptns['_sPgLnkCustTxt'] ) ? $itemOptns['_sPgLnkCustTxt'][0] : '';

    if( !abcfl_html_isblank( $itemTxtCust ) ) { 
        //$itemTxt = $itemTxtCust;
        $parLP['lineTxt'] = $itemTxtCust; 
    } 
    //-------------------------------------------------
    // Changed. Link text is required now.
    //if( abcfl_html_isblank( $itemTxt ) ) {  return ''; }
    if( abcfl_html_isblank( $parLP['lineTxt'] ) ) {  return ''; }
    //-------------------------------------------------
    ...
    $aTag = abcfl_html_a_tag_nb( $lnkParts['href'], $parLP['lineTxt'], $lnkParts['target'], $par['lnkCls'], $par['lnkStyle'], '', $lnkParts['onclick'], $lnkParts['args'] );
